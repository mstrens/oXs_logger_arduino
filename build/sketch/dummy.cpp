#line 1 "c:\\Data\\oXs_logger_arduino\\dummy.cpp"
#ifdef DUMMY
/**
...
struct block_t {            // block_t has a size of just 512 bytes
  uint16_t count;
  uint16_t overrun;
  data_t data[DATA_DIM];  // data values ; size is just below 512
  uint8_t fill[FILL_DIM];  
};

const uint8_t QUEUE_DIM = BUFFER_BLOCK_COUNT + 2; // number of block + 2 (e.g. 10  = 8+2)

block_t* emptyQueue[QUEUE_DIM];    // e.g. 10 pointeurs de block_t (de 512) 
uint8_t emptyHead;
uint8_t emptyTail;
...
// Advance queue index.
inline uint8_t queueNext(uint8_t ht) {          // roll
...
void logData() {
  ...
  block_t block[BUFFER_BLOCK_COUNT]; // allocate 8 * 512 bytes
  block_t* curBlock = 0;
  Serial.println();
  ...
  if (!binFile.createContiguous(sd.vwd(),
                                TMP_FILE_NAME, 512 * FILE_BLOCK_COUNT)) {  // crée un très gros fichier
